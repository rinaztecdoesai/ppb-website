...
This site is plain static HTML/CSS/JS. The repo (`thejfdesign/ppb-website`) is the single source of truth; pushing to `main` deploys through Sevalla.

Sevalla must publish a **clean generated directory**, not the raw repo root, so internal docs, GitHub workflows, env examples, and Python blog tooling are not publicly reachable.

## Sevalla Static Site — settings
Sevalla dashboard → **Static Sites → Settings → Build**:
...
> **Why a clean publish directory:** Sevalla serves the configured publish directory directly. The build script copies only visitor-facing static files into `_sevalla_public`, then fails if private/tooling files such as `.github/`, `.env.example`, `BLOG-SETUP.md`, or `lp/shared/*.py` leak into the artifact.
...
## Sevalla — verified 18 Jun 2026; build artifact update pending
Live deploy was verified at **https://ppb-website-1rtus.kinsta.page/** with build OFF and publish dir `.`; after the Kereeb blog merge, update Sevalla to build ON and publish `_sevalla_public`.
